...
Repeated SYN probes across multiple destination ports matched the behavior generated by the Nmap SYN scan. The packet capture showed the network pattern while the Nmap activity identified what produced it.
...
### TCP responses reveal scan results

SYN ACK and RST responses showed how the target reacted to the probes. Reading those responses helped distinguish listening services from ports that refused the connection.
